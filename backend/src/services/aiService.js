...
КРИТИЧЕСКИ ВАЖНО - ПРАВИЛА ДВИЖЕНИЯ ФИГУР:
- ПЕШКИ: двигаются вперед на 1 поле, атакуют по диагонали вперед на 1 поле
- Белая пешка на e5 может атаковать d6 и f6 (НЕ d4 или f4!)
- Черная пешка на d5 может атаковать c4 и e4 (НЕ c6 или e6!)
- ВАЖНО: пешка НЕ может "отступить" или атаковать назад!

АНАЛИЗ АТАК:
- Если пешка атакует фигуру, эта фигура РЕАЛЬНО под угрозой
- Чтобы спасти атакованную фигуру: убрать её, заблокировать атаку, или атаковать атакующую фигуру
- НЕ путай "поддержку" с "защитой от конкретной атаки"
...
ИНСТРУКЦИИ:
1. Внимательно прочитай FEN и определи расположение ВСЕХ фигур.
2. Проверь, есть ли шах, мат, или тактические угрозы.
3. КРИТИЧНО: Если твоя фигура под атакой, проверь ВСЕ способы атаки противника!
4. НЕ делай ход, который не решает проблему атакованной фигуры!
5. ВЫБЕРИ ОДИН ИЗ ДОСТУПНЫХ ХОДОВ. Это обязательно.
6. Используй стандартную нотацию: от_клетка к_клетка (например: e2e4, g1f3).
7. В объяснении ТОЧНО укажи, какую фигуру ты двигаешь и почему этот ход безопасен.

ФОРМАТ ОТВЕТА:
ХОД: [ваш_ход]
ОБЪЯСНЕНИЕ: [какую фигуру двигаешь и почему]
НОВАЯ_СТРАТЕГИЯ: [обновленная стратегия на следующие ходы]`;
      ...
      // Извлекаем объяснение и новую стратегию - пробуем разные форматы
